...
| `room` | 보낸 시점의 방. 이후 어느 쪽이 방을 옮겨도 바뀌지 않음 |
...
**방 범위.** 세션은 방 하나에 속하고, `list_peers`와 `ask_peer`는 같은 방 안으로만 범위가 좁혀집니다. 이것도 클라이언트를 믿지 않는다는 같은 원칙입니다 — 채널 서버가 방을 속여 다른 방을 조회하지 못하도록 브로커가 판정합니다. 다만 **방은 접근 통제가 아닙니다.** 멤버 목록도 초대도 없고, 이름을 아는 사람은 누구나 들어올 수 있습니다. `list_rooms`는 방 경계를 의도적으로 넘어서, 모든 방의 이름과 주제를 누구에게나 보여줍니다. 방은 대화 맥락을 묶는 수단이지 민감한 내용을 안전하게 두는 곳이 아닙니다 — 자세한 사용법은 [USAGE.md](USAGE.md#2-방-고르기)를 보세요.
...
**git 저장소**(`claude plugin marketplace add nugulhie/claude-chat-session`)로 등록하면 클론본을 캐시에 두고 거기서 실행합니다. 배포된 상태를 그대로 재현하려면 이쪽을 씁니다. 실행 경로는 `claude mcp list`로 확인할 수 있습니다.
